...
const LiveModeOverlay: React.FC<LiveModeOverlayProps> = ({ currentPdf, onClose, onTranscription }) => {
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      try {
        // Resume context on user action (this effect runs on mount which is triggered by button click)
        ...
        const sessionPromise = ai.live.connect({
          ...
          config: {
            ...
            speechConfig: {
              voiceConfig: { prebuiltVoiceConfig: { voiceName: 'Puck' } }, // Try a different voice for clarity
            },
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-2xl flex flex-col items-center justify-center p-6 text-white text-center">
      ...
      <div className="max-w-md">
        <h2 className="text-3xl font-bold mb-4">
          {isConnecting ? 'جاري الاتصال...' : isSpeakerActive ? 'نبراس كاعد يشرح...' : 'اسمعك، تفضل بسؤالك'}
        </h2>
